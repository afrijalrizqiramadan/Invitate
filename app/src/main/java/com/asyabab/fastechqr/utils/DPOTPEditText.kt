...
import com.asyabab.fastechqr.utils.color
import com.asyabab.fastechqr.utils.hideSoftKeyboard
import com.asyabab.fastechqr.utils.strokedBackground

class DPOTPEditText(val mEditTextList: Array<EditText?>, val context: FragmentActivity) {
    init {
    ...
    inner class CodeTextWatcher internal constructor(private val aCurrentIndex: Int) : TextWatcher {
        ...
        override fun afterTextChanged(s: Editable) {
            ...
            if (text.length == 1) {
                mEditTextList[aCurrentIndex]?.strokedBackground(
                    0,
                    strokeColor = context.color(R.color.dp_colorPrimary)
                )
                moveToNext()
            } else if (text.isEmpty()) {
                mEditTextList[aCurrentIndex]?.strokedBackground(
                    0,
                    strokeColor = context.color(R.color.dp_view_color)
                )
                moveToPrevious()
            }
        }
