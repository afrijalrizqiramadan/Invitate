...
class DPSwipeLayout : ViewGroup {
    ...
    private val dragCallback = object : ViewDragHelper.Callback() {
        ...
        override fun onViewPositionChanged(
            ...
            if (dx > 0) {
                ...
                if (leftView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (leftView!!.right - stickyBound > 0 && leftView!!.right - stickyBound - dx <= 0)
                            swipeListener!!.onLeftStickyEdge(this@DPSwipeLayout, true)
                    }
                }
                ...
                if (rightView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (rightView!!.left + stickyBound > width && rightView!!.left + stickyBound - dx <= width)
                            swipeListener!!.onRightStickyEdge(this@DPSwipeLayout, true)
                    }
                }
            } else if (dx < 0) {
                //move to left
                ...
                if (leftView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (leftView!!.right - stickyBound <= 0 && leftView!!.right - stickyBound - dx > 0)
                            swipeListener!!.onLeftStickyEdge(this@DPSwipeLayout, false)
                    }
                }
                ...
                if (rightView != null) {
                    ...
                    if (stickyBound != LayoutParams.STICKY_NONE) {
                        if (rightView!!.left + stickyBound <= width && rightView!!.left + stickyBound - dx > width)
                            swipeListener!!.onRightStickyEdge(this@DPSwipeLayout, false)
                    }
                }
            }
        }
        ...
        private fun onMoveRightReleased(child: View, dx: Int, xvel: Float): Boolean {
            ...
            if (dx > 0 && xvel >= 0 && leftViewClampReached(lp)) {
                if (swipeListener != null) {
                    swipeListener!!.onSwipeClampReached(this@DPSwipeLayout, true)
                }
                return true
            }
        ...
        private fun onMoveLeftReleased(child: View, dx: Int, xvel: Float): Boolean {
            ...
            if (dx < 0 && xvel <= 0 && rightViewClampReached(lp)) {
                if (swipeListener != null) {
                    swipeListener!!.onSwipeClampReached(this@DPSwipeLayout, false)
                }
                return true
            }
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            val a = context.obtainStyledAttributes(attrs, R.styleable.DPSwipe_Layout)
            if (a.hasValue(R.styleable.DPSwipe_Layout_DPswipe_enabled)) {
                isLeftSwipeEnabled = a.getBoolean(R.styleable.DPSwipe_Layout_DPswipe_enabled, true)
                isRightSwipeEnabled = a.getBoolean(R.styleable.DPSwipe_Layout_DPswipe_enabled, true)
            }
            if (a.hasValue(R.styleable.DPSwipe_Layout_DPleft_swipe_enabled)) {
                isLeftSwipeEnabled =
                    a.getBoolean(R.styleable.DPSwipe_Layout_DPleft_swipe_enabled, true)
            }
            if (a.hasValue(R.styleable.DPSwipe_Layout_DPright_swipe_enabled)) {
                isRightSwipeEnabled =
                    a.getBoolean(R.styleable.DPSwipe_Layout_DPright_swipe_enabled, true)
            }
     ...
     *
     *
     * Ignores [DPSwipeLayout.isSwipeEnabled] and [DPSwipeLayout.isLeftSwipeEnabled]
     */
     ...
     *
     *
     * Ignores [DPSwipeLayout.isSwipeEnabled] and [DPSwipeLayout.isRightSwipeEnabled]
     */
    ...
    private fun startScrollAnimation(
        ...
        if (dragHelper!!.settleCapturedViewAt(targetX, view.top)) {
            ...
            if (moveToClamp && swipeListener != null) {
                swipeListener!!.onSwipeClampReached(this@DPSwipeLayout, toRight)
            }
        }
    }
    ...
    private inner class SettleRunnable internal constructor(
        ...
        override fun run() {
            if (dragHelper != null && dragHelper!!.continueSettling(true)) {
                ...
                if (moveToClamp && swipeListener != null) {
                    swipeListener!!.onSwipeClampReached(this@DPSwipeLayout, moveToRight)
                }
            }
        }
    }
    ...
    class LayoutParams : ViewGroup.LayoutParams {
        ...
        constructor(c: Context, attrs: AttributeSet) : super(c, attrs) {
            ...
            val a = c.obtainStyledAttributes(attrs, R.styleable.DPSwipe_Layout)

            val N = a.indexCount
            for (i in 0 until N) {
                val attr = a.getIndex(i)
                if (attr == R.styleable.DPSwipe_Layout_DPswipe_gravity) {
                    gravity = a.getInt(attr, CENTER)
                } else if (attr == R.styleable.DPSwipe_Layout_DPsticky) {
                    sticky = a.getLayoutDimension(attr, STICKY_SELF)
                } else if (attr == R.styleable.DPSwipe_Layout_DPclamp) {
                    clamp = a.getLayoutDimension(attr, CLAMP_SELF)
                } else if (attr == R.styleable.DPSwipe_Layout_DPbring_to_clamp) {
                    bringToClamp = a.getLayoutDimension(attr, BRING_TO_CLAMP_NO)
                } else if (attr == R.styleable.DPSwipe_Layout_DPsticky_sensitivity) {
    ...
    interface OnSwipeListener {
        fun onBeginSwipe(swipeLayout: DPSwipeLayout, moveToRight: Boolean)

        fun onSwipeClampReached(swipeLayout: DPSwipeLayout, moveToRight: Boolean)

        fun onLeftStickyEdge(swipeLayout: DPSwipeLayout, moveToRight: Boolean)

        fun onRightStickyEdge(swipeLayout: DPSwipeLayout, moveToRight: Boolean)
    }

    companion object {

        private val TAG = DPSwipeLayout::class.java.simpleName
