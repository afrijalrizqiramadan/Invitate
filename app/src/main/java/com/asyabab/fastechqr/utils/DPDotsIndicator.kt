...
class DPDotsIndicator(context: Context, attrs: AttributeSet? = null) :
    ...
    init {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.DPDotsIndicator)
        ...
        try {
            intrinsicWidth = ta.getDimensionPixelSize(R.styleable.DPDotsIndicator_dot_width, -1)
            intrinsicHeight = ta.getDimensionPixelSize(R.styleable.DPDotsIndicator_dot_height, -1)
            intrinsicMargin = ta.getDimensionPixelSize(R.styleable.DPDotsIndicator_dot_margin, -1)
            intrinsicOrientation = ta.getInt(R.styleable.DPDotsIndicator_dp_dots_orientation, -1)
            intrinsicGravity = ta.getInt(R.styleable.DPDotsIndicator_dp_dots_orientation, -1)

            this.animatorResId = ta.getResourceId(
                R.styleable.DPDotsIndicator_dots_animator,
                R.anim.dp_scale_with_alpha
            )
            this.animatorReverseResId =
                ta.getResourceId(R.styleable.DPDotsIndicator_dots_animator_reverse, 0)
            this.backgroundResId = ta.getResourceId(
                R.styleable.DPDotsIndicator_dot_drawable,
                R.drawable.dp_bg_black_dot
            )
            this.unselectedBackgroundId = ta.getResourceId(
                R.styleable.DPDotsIndicator_dot_drawable_unselected,
                this.backgroundResId
            )
            this.dotTint = ta.getColor(R.styleable.DPDotsIndicator_dot_tint, 0)
        } finally {
            ta.recycle()
        }
        ...
        indicatorBackgroundResId =
            if (this.backgroundResId == 0) R.drawable.dp_bg_black_dot else this.backgroundResId
